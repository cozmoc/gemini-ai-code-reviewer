...
def analyze_code(parsed_diff: List[Dict[str, Any]], pr_details: PRDetails) -> List[Dict[str, Any]]:
    ...
    # Sort by severity descending, default to 0
    ...
    for r in ai_responses:
        try:
            ...
            if 1 <= ln <= hunk.source_length:
                ...
                comments.append({'path':file.path,'position':ln,'body':body,'severity':sev})
        except:
            continue
    return comments
...
def create_review_comment(owner: str, repo: str, pr_num: int, comments: List[Dict[str, Any]]):
    ...
    pr.create_review(body="AI Review Comments", comments=[{k:v for k,v in c.items() if k!='severity'} for c in comments], event="COMMENT")
...
def parse_diff(diff_str: str) -> List[Dict[str, Any]]:
    ...
    for line in diff_str.splitlines():
        if line.startswith('diff --git'):
            current and files.append(current)
            current = {'path':'','hunks':[]}
        elif line.startswith('--- a/') and current: current['path'] = line[6:]
        elif line.startswith('+++ b/') and current: current['path'] = line[6:]
        elif line.startswith('@@') and current:
            hunk = {'lines':[]}; current['hunks'].append(hunk)
        elif hunk is not None: hunk['lines'].append(line)
    current and files.append(current)
    return files
...
def main():
    ...
    if not data.get('issue',{}).get('pull_request'): return
    ...
    comments and create_review_comment(pr.owner, pr.repo, pr.pull_number, comments)
